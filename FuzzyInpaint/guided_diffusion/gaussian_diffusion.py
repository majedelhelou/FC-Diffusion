...
class GaussianDiffusion:
    ...
    def __init__(self, *, betas, model_mean_type, model_var_type, loss_type, rescale_timesteps=False, conf=None):
    ...
    def p_sample(self, model, x, t, clip_denoised=True, denoised_fn=None, cond_fn=None, model_kwargs=None, conf=None,
                    meas_fn=None, pred_xstart=None, idx_wall=-1):
        """
        ...
        if conf.inpa_inj_sched_prev:
            ...
            if pred_xstart is not None:
                ...
                if gt_keep_mask is None: #handled for ood externally in fuzzy_inpaint
                ...
                if conf.inpa_inj_sched_prev_cumnoise: #
                    ...
                    weighed_gt = gt_part + noise_part #G_n

                # std, mu := self.p_mean_variance(model, x, t, clip_denoised=clip_denoised, denoised_fn=denoised_fn, model_kwargs=model_kwargs,)
                # Fuzzy fuse & renormalize post. probability distribution:
                x = (gt_keep_mask * weighed_gt + (1 - gt_keep_mask) * x)
                if not conf['fuzzy_off']:
                    std_fac = th.sqrt(1-2*gt_keep_mask+2*gt_keep_mask*gt_keep_mask)
                    mu = th.sqrt(alpha_cumprod) * gt
                    x = (x-mu)/std_fac+mu


        out = self.p_mean_variance(model, x, t, clip_denoised=clip_denoised, denoised_fn=denoised_fn, model_kwargs=model_kwargs,)
        #out["mean"] == gt_weight * gt?
        ...
        result = {"sample": sample, "pred_xstart": out["pred_xstart"], 'gt': model_kwargs.get('gt')}

        return result

    def p_sample_loop(self, model, shape, noise=None, clip_denoised=True, denoised_fn=None, cond_fn=None,
                        model_kwargs=None, device=None, progress=True, return_all=False, conf=None):
        """
        ...
        for sample in self.p_sample_loop_progressive(model, shape, noise=noise, clip_denoised=clip_denoised, 
                denoised_fn=denoised_fn, cond_fn=cond_fn, model_kwargs=model_kwargs, device=device, progress=progress, conf=conf):
            final = sample
    ...
    def p_sample_loop_progressive(self, model, shape, noise=None, clip_denoised=True, denoised_fn=None, cond_fn=None,
                model_kwargs=None, device=None, progress=False, conf=None):
        """
            ...
            for t_last, t_cur in time_pairs:
                ...
                if t_cur < t_last:  # reverse
                    with th.no_grad():
                        ...
                        out = self.p_sample(model, image_after_step, t_last_t, clip_denoised=clip_denoised, denoised_fn=denoised_fn,
                            cond_fn=cond_fn, model_kwargs=model_kwargs, conf=conf, pred_xstart=pred_xstart)
